    ...
    def compute(self, raw_records, raw_records_simu, start, end):
        ...
        if len(r_simu):
            ...
            if self.pmt_pulse_filter:
                # Filter to concentrate the PMT pulses
                strax.filter_records(
                    r_simu, np.array(self.pmt_pulse_filter))

            le, re = self.save_outside_hits
            r_simu = strax.cut_outside_hits(r_simu, hits_simu,
                                       left_extension=le,
                                       right_extension=re)
...
def shift_salt_channels(r, n_channel_shift=SCHANNEL_STARTS_AT):
    ...
    :param n_channel_shift: number of channels to shift, default is SCHANNEL_STARTS_AT
