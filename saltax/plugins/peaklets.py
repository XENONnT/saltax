...
class SPeaklets(straxen.Peaklets):
    ...
    def setup(self):
        ...
        self.to_pe = np.zeros(SCHANNEL_STARTS_AT + self.n_tpc_pmts, dtype=np.float64)
        self.to_pe[: self.n_tpc_pmts] = self.to_pe_data
        self.to_pe[SCHANNEL_STARTS_AT:] = self.to_pe_simu

        # Get the hitfinder thresholds
        self.hit_thresholds = np.zeros(SCHANNEL_STARTS_AT + self.n_tpc_pmts, dtype=np.int64)
        self.hit_thresholds[: self.n_tpc_pmts] = self.hit_min_amplitude
        self.hit_thresholds[SCHANNEL_STARTS_AT:] = self.hit_min_amplitude
    ...
    def compute(self, records, start, end):
        ...
        mask_salted_lone_hits = result["lone_hits"]["channel"] >= SCHANNEL_STARTS_AT
        result["lone_hits"]["channel"][mask_salted_lone_hits] -= SCHANNEL_STARTS_AT
